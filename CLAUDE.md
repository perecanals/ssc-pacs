...
│   ├── scripts/              # Diagnostic/testing utilities
...
│   └── orthanc_users.json    # Generated by manage_users.py — never edit manually
...
docker compose up -d
docker compose down
./check_status.sh          # status, API, plugins
```
...
python manage_users.py add <user> [--admin]
python manage_users.py passwd <user>
python manage_users.py remove <user>
# After any change:
...
python scripts/cleanup_loose_dicoms.py                  # dry-run
python scripts/cleanup_loose_dicoms.py --execute        # apply
python scripts/cleanup_loose_dicoms.py --patient 4-0551 # limit to one patient
...
python scripts/archive_all_series.py --patient 4-0551   # idempotent retry

# On-demand DICOM → NIFTI (cold_path_cache mode skips auto-NIFTI generation)
python scripts/dicom_to_nifti.py --dir /path/to/DICOM
python scripts/dicom_to_nifti.py --archive /path/to/DICOM.tar.zst --out /tmp/x.nii.gz
python scripts/dicom_to_nifti.py --series-uid <uid> --warm-if-cold
```
...
**Auth:**
- Orthanc: `orthanc_users.json` (plaintext, generated by `manage_users.py`).
- Companion: bcrypt hashes in `users` table + JWT cookies; service-to-service Orthanc calls use `ORTHANC_ADMIN_USER`/`ORTHANC_ADMIN_PASSWORD` from `.env`.
- `manage_users.py` is the single provisioning tool for both auth systems.
...
**Archive format:** files are stored flat at the archive root (matching `archive_all_series.py` convention — no `DICOM/` subdirectory wrapper).
...
- `teardown.sh` is destructive and sources `.env` from two levels above the repo root (`../../.env`), not the repo-root `.env` — use with care.
- `orthanc_users.json` must not be edited manually; always use `manage_users.py`.
