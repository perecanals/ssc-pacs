> **Historical document.** Original web app plan (Docker, `proces` DB, `flowcat_*`). Current Web App: [`../reference/web_app.md`](../reference/web_app.md) and [`../reference/runtime_and_config.md`](../reference/runtime_and_config.md).

# Implementation Plan: Series Annotation Web App App
...
### Solution
A self-contained FastAPI web app app that:
...
├── Web App (:8043)           ← series-level browsing & annotations
└── OHIF (:8042/ohif/)          ← image viewing

Docker
├── Orthanc container (:8042)   ← unchanged
└── Web App container (:8043) ← new, FastAPI
...
### `web-app/app.py` — FastAPI backend
...
### Docker
- `web-app/Dockerfile` — python:3.11-slim + uvicorn
- `web-app/requirements.txt` — fastapi, uvicorn, psycopg2-binary, requests
- Added as `web app` service in `docker-compose.yml` with `network_mode: host`

## 4. Modularity

To remove the web app app entirely:
1. Delete the `web-app/` folder
2. Remove the `web app` service from `docker-compose.yml`
