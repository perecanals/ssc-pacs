...
const sanitizeKeys = (keys) => Array.from(new Set(Array.isArray(keys) ? keys : []));
...
export default function useColumnPrefs(labelDefs, builtinCols, level, initialPrefs = {}) {
  ...
  const [visibleKeys, setVisibleKeys] = useState(() => {
    if (hasSavedPrefs) {
      return sanitizeKeys(initialPrefs.visibleKeys);
    }
