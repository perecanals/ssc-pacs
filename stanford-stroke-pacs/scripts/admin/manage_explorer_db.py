#!/usr/bin/env python3
"""Provision/check the dedicated Data Explorer role; never print credentials.
...
SELECT grants after adding an approved research table.
"""
import argparse
import os
import secrets
...
def provision():
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not existing:
                cur.execute(sql.SQL("CREATE ROLE {} LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS").format(ident))
            ...
            cur.execute(sql.SQL("GRANT CONNECT ON DATABASE {} TO {}").format(sql.Identifier(DB_CONFIG["dbname"]), ident))
            ...
            cur.execute("SELECT c.relname FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace "
                        "WHERE n.nspname='public' AND c.relkind='r' AND c.relname=ANY(%s)", (list(TABLES),))
    ...
    check_role()
    print("Explorer reader provisioned and checked; credentials saved in .env (not displayed).")
...
def main():
    ...
    try:
        ...
        print("Explorer role operation failed. Check database privileges, role ownership and .env write access.", file=sys.stderr)
        return 1
    return 0
