...
src_dir: /path/to/new_cases_root            # one case per subdirectory (see note below)
...
| `anonymize_files` | Header-level de-identification applied to every copied file **and** to the in-memory headers the pipeline reads (so `series_dicom_tags`, `protocolname`, etc. describe the anonymised files, not the identified source). Sets `PatientID` and `StudyID` to the patient id (created when absent), `PatientName` to `Anonymous`, `PatientIdentityRemoved=YES` + `DeidentificationMethod`; blanks (only when present) the institution / patient-demographics / physician / operator / accession / device / `ProtocolName` / comment fields listed in `utils.ANONYMIZE_BLANK_KEYWORDS`; removes `RequestAttributesSequence`, `OtherPatientIDsSequence`, `ReferencedPatientSequence`. **Preserved on purpose:** every UID, `StudyDate`/`StudyTime` and the acquisition timestamps, `StudyDescription`, `SeriesDescription`, geometry and reconstruction tags, and private tags (kept for vendor discrimination; `anonymize_dicom_slice(..., remove_private_tags=True)` exists for callers that want them gone). No pixel redaction — burned-in annotations are out of scope. Identity is unchanged: the pipeline still keys on the source `PatientID`, so a source that must be re-keyed (e.g. an already-anonymised tree) has to be rewritten/staged first. The anonymise copy path re-encodes each file with pydicom, which is slower than a plain copy and disables the size-equality check in `verify_ingested_case`. |
