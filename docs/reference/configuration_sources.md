...
| Data/backup **mountpoints** (boot ordering) | derived from `config.toml` `[storage]` roots + `[backup].backup_root` via `stat -c %m` (per-host override: `deploy.env` `DATA_MOUNTS`/`BACKUP_MOUNTS`) | `RequiresMountsFor=` in the rendered `ssc-web-app`/cold-storage/backup units **and** the `docker.service` drop-in `/etc/systemd/system/docker.service.d/10-ssc-data-mounts.conf` | `install_systemd.sh` substitutes `__DATA_MOUNTS_LINE__`/`__BACKUP_MOUNTS_LINE__` — **re-run the installer after moving data to another filesystem**. Prevents a late `nofail`/LUKS mount racing the stack; see [`cold_storage/runbook.md`](../cold_storage/runbook.md) "No images anywhere after a reboot" |
